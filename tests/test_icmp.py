...
class TestIcmp(unittest.TestCase):
    """Тесты для проверки сборки и валидации ICMP пакетов."""

    def test_icmp_packet_init(self):
        """Проверка правильного сохранения параметров объекта."""
    ...
    def test_icmp_packet_create_request_size(self):
        """Проверка сборки пакетов под разные размеры."""
    ...
    def test_checksum_calculation(self):
        """Проверка ручного расчета контрольной суммы."""
        ...
        self.assertIsInstance(
            packet._calculate_checksum(data_even), int
        )
        self.assertIsInstance(
            packet._calculate_checksum(data_odd), int
        )

    def test_verify_reply_short_data(self):
        """Проверка, что короткие пакеты из сети отбрасываются."""
    ...
    def test_verify_reply_type_0_success(self):
        """Проверка успешного распознавания эхо-ответа."""
        packet = ICMPPacket(pid=1234, seq=5, packet_size=40)
        raw = bytearray(30)
        raw[20] = 0
    ...
    def test_verify_reply_type_0_invalid(self):
        """Проверка отбрасывания пакета с неверным PID."""
        packet = ICMPPacket(pid=1234, seq=5, packet_size=40)
        raw = bytearray(30)
        raw[20] = 0
    ...
    def test_verify_reply_type_11_success(self):
        """Проверка распознавания ответов типа 11."""
        packet = ICMPPacket(pid=1234, seq=5, packet_size=40)
        raw = bytearray(60)
        raw[20] = 11
    ...
    def test_verify_reply_type_11_short(self):
        """Проверка отклонения короткого пакета типа 11."""
        packet = ICMPPacket(pid=1234, seq=5, packet_size=40)
        raw = bytearray(40)
        raw[20] = 11
        self.assertFalse(packet.verify_reply(bytes(raw)))
    ...
    def test_verify_reply_unknown_type(self):
        """Проверка, что неизвестные типы возвращают False."""
        packet = ICMPPacket(pid=1234, seq=5, packet_size=40)
        raw = bytearray(30)
        raw[20] = 3
        self.assertFalse(packet.verify_reply(bytes(raw)))
